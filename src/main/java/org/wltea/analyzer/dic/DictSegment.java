...
class DictSegment implements Comparable<DictSegment>{ // NOTE:htt, 词典树，用于词典查找
	...
	//当前节点上存储的字符
	private Character nodeChar; // NOTE:htt, 记录当前存储的 字符， 如："石"
	//当前节点存储的Segment数目
	 ...
	 * @return Hit 
	 */
	Hit match(char[] charArray , int begin , int length , Hit searchHit){ // NOTE:htt, [begin, begin+lenght]查找该范围内的charArray是否有对应满足数据
	 ...
	 * @param enabled
	 */
	private synchronized void fillSegment(char[] charArray , int begin , int length , int enabled){ // NOTE:htt, 将 中文字符纳入 加载到内存中
		//获取字典表中的汉字对象
	 ...
	 * @return
	 */
	private DictSegment lookforSegment(Character keyChar ,  int create){ // NOTE:htt, 如果小于ARRAY_LENGTH_LIMIT采用数组存储，否则采用map存储
	 ...
	 * @param segmentArray
	 */
	private void migrate(DictSegment[] segmentArray , Map<Character , DictSegment> segmentMap){ // NOTE:htt, 将segmentArray数组内容迁移到 Map中，key为 nodeChar字符
